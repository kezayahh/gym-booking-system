...
@section('title', 'My Profile - City Gymnasium Admin')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-800">
        <i class="fas fa-user-cog text-blue-500 mr-2"></i>My Profile
    </h1>
    <p class="text-gray-600 mt-1">Manage your admin account settings</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-lg shadow-lg p-6">
            <div class="text-center">
                ...
                <div class="mt-4 pt-4 border-t border-gray-200">
                    <span class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-yellow-400 to-orange-500 text-white rounded-full text-sm font-bold">
                ...
                <div class="mt-4 pt-4 border-t border-gray-200">
                    ...
                    <p class="text-sm font-semibold text-gray-800">{{ Auth::user()->created_at->format('M d, Y') }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mt-6">
            <h3 class="text-lg font-bold text-gray-800 mb-4">
                <i class="fas fa-chart-bar text-blue-500 mr-2"></i>Your Activity
            </h3>
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <span class="text-gray-600 text-sm">Total Actions</span>
                    <span class="font-bold text-gray-800">{{ Auth::user()->activityLogs()->count() }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-gray-600 text-sm">This Month</span>
                    <span class="font-bold text-blue-600">{{ Auth::user()->activityLogs()->whereMonth('created_at', now()->month)->count() }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-gray-600 text-sm">Last Login</span>
                    <span class="font-bold text-green-600 text-xs">{{ Auth::user()->updated_at->diffForHumans() }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-2">
            ...
            <form id="profileForm" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="phone" name="phone" value="{{ Auth::user()->phone ?? 'Not provided' }}" readonly
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            <i class="fas fa-shield-alt text-gray-400 mr-1"></i>Role
                        </label>
                        <input type="text" value="Administrator" readonly
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-yellow-50 text-yellow-800 font-bold">
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="text" id="address" name="address" value="{{ Auth::user()->address ?? 'Not provided' }}" readonly
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50">
                    </div>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            <h3 class="text-xl font-bold text-gray-800 mb-6">
                <i class="fas fa-shield-alt text-blue-500 mr-2"></i>Security Settings
            </h3>
...
document.getElementById('profileForm').addEventListener('submit', function(e) {
    ...
    fetch('/admin/profile/update-info', {
        method: 'POST',
        headers: {
...
document.getElementById('passwordForm').addEventListener('submit', function(e) {
    ...
    fetch('/admin/profile/update-password', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
